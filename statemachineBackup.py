        ...
        for i, die_rect in enumerate(hand_rects or []):
            if i < len(rolls) and die_rect.collidepoint(mouse_pos):
                ...
                if non_color_enh:  # Only show if has non-color enh
                    enh_desc = ', '.join(data.ENH_DESC.get(e, e) for e in non_color_enh)
...
class ShopState(State):
    ...
    def draw(self):
        ...
        if constants.DEBUG:
            ...
            if DEBUG and constants.DEBUG_MENU_IN_SHOP:
    ...
    def handle_event(self, event):
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if DEBUG and constants.DEBUG_MENU_IN_SHOP and self.debug_button_rect.collidepoint(mouse_pos):
